...
export class AuthService {
 ...
 private tokenExpirationTimer: any;
  ...
  signup(email: string, password: string) {
    return this.http
      .post<AuthResponseData>(
        'https://www.googleapis.com/identitytoolkit/v3/relyingparty/signupNewUser?key=AIzaSyCbcR_fmDKPKK1PMl1sPNWkPeOoTmzThZI',
        {
          email: email,
          password: password,
  ...
  login(email:string, password: string) {
    return this.http
    .post<AuthResponseData>(
      'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyCbcR_fmDKPKK1PMl1sPNWkPeOoTmzThZI',
      {
        email: email,
        password: password,
        returnSecureToken: true
